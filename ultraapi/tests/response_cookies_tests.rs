...
async fn cookie_mixed() -> CookieResponse<Message> {
    ...
    .cookie_options("session", "abc123", |opts| {
        opts.http_only().secure().path("/")
    })
    .cookie_options("preference", "dark", |opts| {
        opts.same_site_lax().max_age(86400)
    })
...
async fn test_multiple_cookies() {
    ...
    assert!(
        all_cookies.contains("session=abc123"),
        "Missing session cookie: {}",
        all_cookies
    );
    assert!(
        all_cookies.contains("user_id=42"),
        "Missing user_id cookie: {}",
        all_cookies
    );
    assert!(
        all_cookies.contains("theme=dark"),
...
async fn test_cookie_mixed_options() {
    ...
    assert!(
        all_cookies.contains("tracking=xyz789"),
        "Missing tracking cookie"
    );
    ...
    assert!(
        all_cookies.contains("preference=dark"),
        "Missing preference cookie"
    );
    assert!(all_cookies.contains("SameSite=Lax"), "Missing SameSite=Lax");
    assert!(
        all_cookies.contains("Max-Age=86400"),
        "Missing Max-Age=86400"
    );

    // Fourth cookie (expires)
    assert!(
        all_cookies.contains("persistent=value1"),
        "Missing persistent cookie"
    );
