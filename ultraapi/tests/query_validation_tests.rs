...
async fn test_query_validation_failure() {
    ...
    assert!(
        !body["details"].as_array().unwrap().is_empty(),
...
async fn test_query_parse_failure() {
    ...
    let response = client
        .get("/__test_query_validation?limit=notanumber")
        .await;
...
async fn test_query_required_field_missing() {
    ...
    assert_eq!(
        response.status(),
        400,
        "Missing required field should return 400 (parse error)"
    );
}
...
async fn test_query_required_field_provided() {
    ...
    assert_eq!(
        response.status(),
        200,
        "Providing required field should return 200"
    );
