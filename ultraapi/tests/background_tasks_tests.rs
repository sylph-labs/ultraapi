...
mod integration {
    ...
    use ultraapi::response_tasks::{BackgroundTasks, response_task_middleware};
    ...
    async fn test_task_runs_after_response() {
        ...
        let app = Router::new()
            .route("/test", get(move |tasks: BackgroundTasks| {
        ...
        // タスクが実行されたことを確認
        assert!(executed.load(Ordering::SeqCst), "Background task should have run after response");
    }
    ...
    async fn test_multiple_tasks() {
        ...
        assert!(counter.load(Ordering::SeqCst), "All background tasks should have run");
    }
