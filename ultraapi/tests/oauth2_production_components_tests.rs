...
fn test_token_response_with_refresh_token() {
    ...
    assert_eq!(response.refresh_token, Some("refresh_token_456".to_string()));
}
...
fn test_token_data_new() {
    let token_data = TokenData::new("user123".to_string(), vec!["read".to_string(), "write".to_string()]);
...
fn test_token_data_with_claim() {
    let token_data = TokenData::new("user123".to_string(), vec![])
        .with_claim("email", serde_json::Value::String("user@example.com".to_string()));
    
    assert!(token_data.claims.contains_key("email"));
    assert_eq!(token_data.claims["email"], serde_json::Value::String("user@example.com".to_string()));
}
...
async fn test_opaque_token_validator_valid_token() {
    let validator = OpaqueTokenValidator::new()
        .add_token("valid-token-123", "user1", vec!["read".to_string()]);
...
async fn test_opaque_token_validator_invalid_token() {
    ...
    assert!(matches!(result.unwrap_err(), OAuth2AuthError::TokenNotFound));
}

#[tokio::test]
async fn test_opaque_token_validator_validate_scopes_success() {
    let validator = OpaqueTokenValidator::new()
        .add_token("valid-token-123", "user1", vec!["read".to_string(), "write".to_string()]);
...
async fn test_opaque_token_validator_remove_token() {
    let validator = OpaqueTokenValidator::new()
        .add_token("token1", "user1", vec!["read".to_string()]);
...
async fn test_opaque_token_validator_clone() {
    let validator1 = OpaqueTokenValidator::new()
        .add_token("token1", "user1", vec!["read".to_string()]);
...
async fn test_token_endpoint_example() {
    ...
    let validator = OpaqueTokenValidator::new()
        .add_token("user_pass_valid", "user1", vec!["read".to_string(), "write".to_string()]);
    ...
    let response = TokenResponse::with_scopes(
        "generated_token_abc".to_string(),
        3600,
        form.scopes()
    );
...
fn test_oauth2_module_exports() {
    ...
    use ultraapi::oauth2::{OAuth2PasswordRequestForm, TokenResponse, OAuth2ErrorResponse};
