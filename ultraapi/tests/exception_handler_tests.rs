...
use std::sync::Arc;
use axum::{body::Body, http::Request, response::IntoResponse, http::StatusCode, routing::get, Router};
...
fn make_error_handler() -> CustomErrorHandler {
    Arc::new(|_state: AppState, _req: Request<Body>, _error: Box<dyn std::any::Any + Send + 'static>| {
...
async fn test_raw_router_middleware_transforms_error() {
    ...
    let app = Router::new()
        .route("/error", get(|| async { (StatusCode::INTERNAL_SERVER_ERROR, "Original error").into_response() }))
        .layer(axum::middleware::from_fn(|req, next: axum::middleware::Next| {
            async move {
...
async fn test_error_handler_catches_custom_error() {
    ...
    let error_handler: CustomErrorHandler = Arc::new(|_state: AppState, _req: Request<Body>, _error: Box<dyn std::any::Any + Send + 'static>| {
        Box::pin(async {
            (StatusCode::BAD_REQUEST, "Custom error").into_response()
        })
    });
...
async fn test_error_handler_with_closure() {
    ...
    let handler: CustomErrorHandler = Arc::new(|_state: AppState, _req: Request<Body>, _error: Box<dyn std::any::Any + Send + 'static>| Box::pin(async move {
...
async fn test_error_handler_transforms_500_response() {
    ...
    let error_handler: CustomErrorHandler = Arc::new(|_state: AppState, _req: Request<Body>, error: Box<dyn std::any::Any + Send + 'static>| {
        Box::pin(async move {
    ...
    let response = app
        .oneshot(Request::builder().uri("/internal-error").body(Body::empty()).unwrap())
        .await
        .unwrap();
...
async fn test_error_handler_transforms_404_response() {
    ...
    let response = app
        .oneshot(Request::builder().uri("/nonexistent-route-xyz").body(Body::empty()).unwrap())
        .await
        .unwrap();
...
async fn test_error_handler_receives_correct_status_code() {
    ...
    // Make request
    let response = app
        .oneshot(Request::builder().uri("/service-unavailable").body(Body::empty()).unwrap())
        .await
        .unwrap();
