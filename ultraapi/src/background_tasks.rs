...
//! # 概要
//! 
//! このモジュールには2種類のバックグラウンドタスク機能があります：
//! 
//! 1. **このモジュール (background_tasks.rs)**: サーバ常駐型のタスク管理
//!    - アプリケーション起動中常に動作するタスク
//!    - 定期実行（Interval）タスク
//!    - グレースフルシャットダウン対応
//!    - グローバルなタスクリスト管理
//! 
//! 2. **response_tasks.rs**: FastAPI互換のレスポンス後実行タスク
//!    - 各リクエストのレスポンス送信後に実行
//!    - ハンドラ内でタスクを追加
//!    - リクエストごとに独立したタスク実行
//! 
//! # 使用場面
//! 
//! - サーバ常駐タスク: DBクリーンアップ、定期的なデータ同期、キャッシュ更新等
//! - レスポンス後タスク: メール送信、処理完了通知、非同期ロギング等
//! 
//! # モジュール選択の目安
//! 
//! - リクエスト處理とは獨立した常駐タスクが必要な場合 → background_tasks
//! - リクエスト處理に付随する非同期処理を別で行い、レスポンスを速く返したい場合 → response_tasks
